...
tell application "Finder"
	tell application "System Events"
		...
		tell PT
			...
			set ASOpen to count (windows whose name contains "Audio Suite: MAutoAlign")
			-- Check whether there is an open track plug-in window.
			set TROpen to count (windows whose name contains "Plug-in: MAutoAlign")
			
			if (ASOpen + TROpen) < 2 then
				display dialog "MAutoAlign track plug-in and Audio Suite plug-in should be open." buttons {"OK"} default button {"OK"} with icon caution with title "AppleScript error"
				return
			end if
			
			-- Bring track plug-in to front.
			set trackWindow to (1st window whose name contains "Plug-in: MAutoAlign")
			...
			set ASWindow to (1st window whose name contains "Audio Suite: MAutoAlign")
