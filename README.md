...
[tracks_to_table.py](../blob/master/Text%20session%20info%20to%20table/tracks_to_table.py) script converts `.txt` file, that could be exported from Pro Tools using *Export Session Info as Text* command, into `.html` or `.csv` file. HTML file is ready to print, and CSV could be easily edited in Numbers.app or Excel. Also an [macOS app](../blob/master/Text%20session%20info%20to%20table/EDL-tools_macos_app.zip) with drag-n-drop support is provided.
#### Usage
```
